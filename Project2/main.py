...
if __name__ == '__main__':
    ...
    if arg == "Log":
        ...
        dnn_f = NN(X_train_sc, Y_train_onehot, eta=eta_final, lmbd=lmbd_final, epochs=epochs,
        ...
        X_train, X_test, y_train, y_test = train_test_split(X, z, test_size=1./k)
        ...
        nn_reg.create_biases_and_weights(weight_spread = 0.01, bias_constant = 0.01)
        ...
        i_min, j_min = 0,0
        nn_reg.train(X_train, y_train, learning_rate=1e-4, regularization=1e-2,
                     n_epochs=epochs, batch_size=batch_size)
        y_pred2 = nn_reg.predict(X.T).flatten()

        
        print("MSE sco  re on test set: ", func.MeanSquaredError(np.ravel(ZZ), np.ravel(y_pred2)))
        print("R2  score on test set: ", func.R2_ScoreFunction(np.ravel(ZZ), np.ravel(y_pred2)))
        ...
        surf2 = ax2.plot_surface(XX, YY, y_pred2.reshape(XX.shape), cmap=cm.inferno,
                               linewidth=0, antialiased=False)
